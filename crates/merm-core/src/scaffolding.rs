...
"#,
    ...
    /// Generates code for a Rust module matching node parameters
...
"#,
            ...
            let fields_str = if attributes.is_empty() {
                String::new()
            } else {
                let mut f = String::new();
                for attr in attributes {
                    let clean = attr.trim_start_matches('+').trim_start_matches('-').trim();
                    f.push_str(&format!("    pub {},\n", clean));
                }
                f
            };

            let struct_def = if fields_str.is_empty() {
                format!("pub struct {};\n", name)
            } else {
                format!("pub struct {} {{\n{}}}\n", name, fields_str)
            };
